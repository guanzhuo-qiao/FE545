...
int main()
{
	...
	SimpleBinomialTreeAKI theTreeAKI(Spot, rParam, dParam, Vol, Steps, Expiry, Barrier);
	double americanAKIPrice = theTreeAKI.GetThePrice(americanOption);
	cout <<"amer knock-in price " << americanAKIPrice << "\n";
	...
	SimpleBinomialTreeAKI theNewTreeAKI(Spot, rParam, dParam, Vol, Steps, Expiry, Barrier);

    double euroNewPrice = theNewTree.GetThePrice(europeanOption);
    double americanNewPrice = theNewTree.GetThePrice(americanOption);
	double americanNewPriceAKI = theNewTreeAKI.GetThePrice(americanOption);

    cout << "euro new price " << euroNewPrice << " amer new price " << americanNewPrice << "\n";
	cout << "amer new knock-in price " << americanNewPriceAKI << "\n";
	...
	double averageAmerAKI = 0.5*(americanAKIPrice + americanNewPriceAKI);

    cout << "euro av price " << averageEuro << " amer av price " << averageAmer << "\n";
    cout << "av forward " << averageForward << "\n";
	cout << "av amer knock-in price " << averageAmerAKI << "\n";
